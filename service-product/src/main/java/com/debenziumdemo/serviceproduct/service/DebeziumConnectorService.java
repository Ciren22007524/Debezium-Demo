package com.debenziumdemo.serviceproduct.service;

import com.debenziumdemo.serviceproduct.config.DebeziumConnectorProperties;
...
public class DebeziumConnectorService {
    ...
    public void registerOutboxConnector() {
        ...
        try {
            ...
            restTemplate.exchange(connectorUrl, HttpMethod.DELETE, entity, String.class);
            log.info("Deleted existing Debezium connector: {}", props.getName());
        } catch (Exception e) {
            log.warn("Delete connector skipped or failed (可能不存在): {}", e.getMessage());
        }
